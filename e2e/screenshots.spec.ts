...
// Seed data keyed for userId=1 (the dev-user fallback)
// Matches src/lib/seed-data.ts structure
function buildSeedLocalStorage(): Record<string, string> {
  const now = Date.now()
  const day = 86400000
  const hour = 3600000
...
async function injectSeedData(page: Page) {
...
test.describe('Visual screenshots', () => {
  ...
  test('desktop - outing detail', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 900 } })
    const page = await ctx.newPage()
    await injectSeedData(page)
    await page.goto(`${BASE}#outings`)
    await page.waitForTimeout(1500)
    const firstRow = page.locator('button:has-text("species")').first()
    await firstRow.click()
    await page.waitForTimeout(2000)
    await page.screenshot({ path: 'screenshots/desktop-outing-detail.png', fullPage: true })
    await ctx.close()
  })

  test('desktop - species detail', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 1280, height: 900 } })
    const page = await ctx.newPage()
    await injectSeedData(page)
    await page.goto(`${BASE}#lifelist`)
  ...
  test('mobile - settings', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 390, height: 844 } })
    const page = await ctx.newPage()
    await injectSeedData(page)
    await page.goto(`${BASE}#settings`)
  ...
  test('mobile - outing detail', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 390, height: 844 } })
    const page = await ctx.newPage()
    await injectSeedData(page)
    await page.goto(`${BASE}#outings`)
    ...
    await firstRow.click()
    await page.waitForTimeout(2000)
    await page.screenshot({ path: 'screenshots/mobile-outing-detail.png', fullPage: true })
    await ctx.close()
  })

  test('mobile - species detail', async ({ browser }) => {
    const ctx = await browser.newContext({ viewport: { width: 390, height: 844 } })
    const page = await ctx.newPage()
    await injectSeedData(page)
    await page.goto(`${BASE}#lifelist`)
