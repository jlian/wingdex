...
export const onRequestPost: PagesFunction<Env> = async context => {
  ...
  try {
    ...
    log?.debug('importCsv.invalidForm', { category: 'Import', resultType: 'Failed', resultSignature: 400, resultDescription: 'Request body could not be parsed as multipart/form-data' })
    return new Response('Invalid form payload', { status: 400 })
  }

  const file = formData.get('file')
  if (!(file instanceof File)) {
    log?.debug('importCsv.missingFile', { category: 'Import', resultType: 'Failed', resultSignature: 400, resultDescription: 'No CSV file found in the "file" form field' })
    return new Response('Missing CSV file', { status: 400 })
  }

  if (file.size > MAX_CSV_SIZE_BYTES) {
    log?.warn('importCsv.tooLarge', { category: 'Import', resultType: 'Failed', resultSignature: 413, resultDescription: `CSV file is ${file.size} bytes, exceeding the ${MAX_CSV_SIZE_BYTES}-byte limit`, properties: { fileSize: file.size, limit: MAX_CSV_SIZE_BYTES } })
