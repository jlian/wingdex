...
import { createLogger, operationNameForRequest } from './lib/log'
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const requestOp = operationNameForRequest(method, pathname)
  ...
  // Handlers attach extra context to the request log here instead of emitting
  // a duplicate "did the obvious thing" log. Middleware merges this into the
  // request lifecycle log's `properties`.
  ...
  context.data.requestProperties = requestProperties

  const start = Date.now()

  // --- HTTP method validation ---
  if (!ALLOWED_METHODS.has(method)) {
    log.warn(requestOp, { resultType: 'Failed', resultSignature: 405, resultDescription: `Method ${method} is not allowed; supported methods are GET, POST, PATCH, DELETE, OPTIONS` })
  ...
  if (hasBodyMethod && rawContentLength !== null) {
    ...
    if (!Number.isFinite(parsedLength) || parsedLength < 0) {
      log.warn(requestOp, { resultType: 'Failed', resultSignature: 400, resultDescription: 'Content-Length header is not a valid non-negative number' })
    ...
    if (parsedLength > 0) {
      ...
      if (parsedLength > limit) {
        log.warn(requestOp, { resultType: 'Failed', resultSignature: 413, resultDescription: `Request body of ${parsedLength} bytes exceeds the ${limit}-byte limit`, properties: { contentLength: parsedLength, limit } })
  ...
  if (pathname.startsWith('/api/auth') || pathname === '/api/health') {
    try {
      ...
      log.info(requestOp, { resultType: response.ok ? 'Succeeded' : 'Failed', resultSignature: response.status, durationMs: Date.now() - start, properties: requestProperties })
      return response
    } catch (err) {
      return handleUnexpectedError(err, log, traceCtx, requestOp, start, requestProperties)
    }
  }
  ...
  if (!session) {
    log.info(requestOp, { resultType: 'Failed', resultSignature: 401, resultDescription: 'No valid session cookie or bearer token', durationMs: Date.now() - start })
  ...
  try {
    const response = withSecurityHeaders(await context.next())
    addTraceHeaders(response, traceCtx)
    authedLog.info(requestOp, { resultType: response.ok ? 'Succeeded' : 'Failed', resultSignature: response.status, durationMs: Date.now() - start, properties: requestProperties })
    return response
  } catch (err) {
    return handleUnexpectedError(err, authedLog, traceCtx, requestOp, start, requestProperties)
  }
}
...
function handleUnexpectedError(
  ...
  log.error(operationName, {
    ...
    properties: { ...requestProperties, error: message, stack },
  })
